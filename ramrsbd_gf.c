/*
 * Galois field utilities
 *
 * Copyright (c) 2024, The littlefs authors.
 * SPDX-License-Identifier: BSD-3-Clause
 */
#include "ramrsbd_gf.h"


// power table, RAMRSBD_GF_POW[x] = g^x
static const uint8_t RAMRSBD_GF_POW[256] = {
    ,
};

// log table, RAMRSBD_GF_LOG[x] = log_g x
static const uint8_t RAMRSBD_GF_LOG[256] = {
    ,
};


// Multiplication in the field
uint8_t ramrsbd_gf_mul(uint8_t a, uint8_t b) {
    // special case for zeros
    if (a == 0 || b == 0) {
        return 0;
    }

    // a*b = g^(log_g a  + log_g b)
    //
    // note our addition can overflow, and there are only
    // 255 elements in multiplication so this is a bit awkward
    //
    uint32_t x = (uint32_t)RAMRSBD_GF_LOG[a]
            + (uint32_t)RAMRSBD_GF_LOG[b];
    if (x > 255) {
        x -= 255;
    }
    return RAMRSBD_GF_POW[x];
}

// Division in the field
uint8_t ramrsbd_gf_div(uint8_t a, uint8_t b) {
    // division by zero is still undefined
    LFS_ASSERT(b != 0);

    // we can avoid some underflow annoyances here by using a slightly
    // different formula
    //
    // a/b = g^(log_g a - log_g b) = g^(log_g a + 255 - log_g b)
    //
    uint32_t x = (uint32_t)RAMRSBD_GF_LOG[a]
            + 255 - (uint32_t)RAMRSBD_GF_LOG[b];
    if (x > 255) {
        x -= 255;
    }
    return RAMRSBD_GF_POW[x];
}

// Exponentiation in the field
uint8_t ramrsbd_gf_pow(uint8_t a, uint32_t e) {
    // special case for a^0
    if (e == 0) {
        return 1;
    // special case for 0^e
    } else if (a == 0) {
        return 0;
    }

    // lookup power in our table
    uint32_t x = ((uint32_t)RAMRSBD_GF_LOG[a] * e) % 255;
    return RAMRSBD_GF_POW[x];
}


